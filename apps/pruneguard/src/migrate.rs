...
use pruneguard_config::{
    AnalysisSeverity, EntrypointsConfig, PruneguardConfig, ResolverConfig, Rule, RuleFilter,
...
pub fn migrate_knip(cwd: &Path, file: Option<&Path>) -> miette::Result<MigrationOutput> {
    ...
    let mut config = PruneguardConfig {
    ...
    if let Some(workspaces) = value.get("workspaces") {
        ...
        if roots.is_empty() {
            warnings.push(
                "knip `workspaces` could not be mapped precisely; review manually.".to_string(),
            );
        } else {
            config.workspaces = Some(WorkspacesConfig { roots, ..WorkspacesConfig::default() });
        }
    }

    if let Some(entry) = value.get("entry") {
        config.entrypoints =
            EntrypointsConfig { include: collect_string_values(entry), ..config.entrypoints };
    }
...
pub fn migrate_depcruise(
    ...
    if let Some(options) = value.get("options") {
        if let Some(tsconfig) = options.get("tsConfig") {
            let resolver =
                ResolverConfig { tsconfig: collect_string_values(tsconfig), ..config.resolver };
        ...
        if options.get("knownViolations").is_some() {
            warnings.push(
                "dependency-cruiser knownViolations are not migrated; use baseline.json."
                    .to_string(),
            );
        }
    }
    ...
    {
        warnings.push(
            "dependency-cruiser config did not expose recognized top-level fields.".to_string(),
        );
    }
    ...
    Ok(MigrationOutput { source: path.to_string_lossy().to_string(), config, warnings })
}
...
fn load_knip_config(cwd: &Path, file: Option<&Path>) -> miette::Result<(String, Value)> {
    ...
    if knip_json.exists() {
        let value = serde_json::from_str(&std::fs::read_to_string(&knip_json).into_diagnostic()?)
            .into_diagnostic()?;
    ...
    if package_json.exists() {
        let value: Value =
            serde_json::from_str(&std::fs::read_to_string(&package_json).into_diagnostic()?)
                .into_diagnostic()?;
...
fn map_depcruise_rule(value: &Value) -> Option<Rule> {
    ...
    Some(Rule {
        name,
        severity,
        comment: value.get("comment").and_then(Value::as_str).map(ToString::to_string),
        from,
        to,
    })
}
...
fn collect_string_values(value: &Value) -> Vec<String> {
    match value {
        ...
        Value::Object(map) => map.values().flat_map(collect_string_values).collect(),
        Value::Bool(_) | Value::Null | Value::Number(_) => Vec::new(),
    }
}

fn resolve_explicit_path(cwd: &Path, file: &Path) -> PathBuf {
    if file.is_absolute() { file.to_path_buf() } else { cwd.join(file) }
}

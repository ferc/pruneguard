...
function run(parsed: Parsed): void {
  ...
  switch (command) {
    ...
    case "init": {
      const output = path.join(process.cwd(), "pruneguard.json");
      fs.writeFileSync(output, JSON.stringify(defaultConfig(), null, 2));
      console.error("Created pruneguard.json");
      return;
    }
    case "print-config": {
      ...
      JSON.stringify(
        {
          ...
          runs: [
            {
              tool: { driver: { name: "pruneguard" } },
              results: findings.map((finding) => ({
                ruleId: finding.code ?? "pruneguard",
                level:
...
function printHelp(): void {
  console.log("pruneguard - Repo truth engine for JS/TS monorepos");
  console.log("");
  console.log("Usage:");
  console.log("  pruneguard [paths...]");
  console.log("  pruneguard scan [paths...]");
  console.log("  pruneguard --focus src/** scan");
  console.log("  pruneguard impact <target>");
  console.log("  pruneguard explain <query>");
  console.log("  pruneguard init");
  console.log("  pruneguard print-config");
  console.log("  pruneguard debug resolve <specifier> --from <file>");
  console.log("  pruneguard debug entrypoints");
  console.log("  pruneguard migrate knip [file]");
  console.log("  pruneguard migrate depcruise [file]");
}

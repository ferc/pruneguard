...
impl SourceAdapter for VueAdapter {
    ...
    fn extract(&self, path: &Path, source: &str) -> Result<AdapterOutput, ExtractError> {
        let facts = extract_vue_facts(path, source);
...
impl SourceAdapter for SvelteAdapter {
    ...
    fn extract(&self, path: &Path, source: &str) -> Result<AdapterOutput, ExtractError> {
        let facts = extract_svelte_facts(path, source);
...
impl SourceAdapter for AstroAdapter {
    ...
    fn extract(&self, path: &Path, source: &str) -> Result<AdapterOutput, ExtractError> {
        let facts = extract_astro_facts(path, source)?;
...
impl SourceAdapter for MdxAdapter {
    ...
    fn extract(&self, path: &Path, source: &str) -> Result<AdapterOutput, ExtractError> {
        ...
        let synthetic_imports = detect_mdx_component_refs(source, &facts);
        Ok(AdapterOutput {
            facts,
            synthetic_imports,
            ..Default::default()

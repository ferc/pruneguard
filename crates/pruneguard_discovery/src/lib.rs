...
impl DiscoveryResult {
    ...
    pub fn package_names(&self) -> FxHashMap<String, String> {
        self.workspaces
            .iter()
            .map(|(name, workspace)| {
                (name.clone(), workspace.manifest.name.clone().unwrap_or_else(|| name.clone()))
            })
            .collect()
    }

    /// Collect tracked files under the project root.
    pub fn collect_files(&self, config: &PruneguardConfig) -> Vec<FileRecord> {
...
pub fn discover(cwd: &Path, config: &PruneguardConfig) -> miette::Result<DiscoveryResult> {
...
fn find_project_root(cwd: &Path) -> PathBuf {
    ...
    loop {
        if current.join("pnpm-workspace.yaml").exists() || current.join("lerna.json").exists() {

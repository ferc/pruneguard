...
impl AnalysisCache {
    ...
    pub fn open(project_root: &Path) -> Result<Self, CacheError> {
        let cache_dir = project_root.join(".oxgraph");
    ...
    pub fn lookup_path_index(&self, relative_path: &Path) -> Result<Option<PathIndexEntry>, CacheError> {
    ...
    fn get_json<T: DeserializeOwned>(
        ...
        let table = read_txn
            ...
            .map_err(|err| CacheError::ReadError(err.to_string()))?;
        let Some(value) = table.get(key).map_err(|err| CacheError::ReadError(err.to_string()))? else {
            return Ok(None);
        };

...
use oxgraph_config::{
    AnalysisSeverity, OxgraphConfig, OwnershipConfig, OverrideConfig, Rule, RuleFilter,
    RulesConfig, TeamConfig,
};
use oxgraph_entrypoints::EntrypointProfile;
use oxgraph_graph::{FileId, GraphBuildResult, ModuleNode, PackageId};
use oxgraph_report::{
    ...
    pub fn evaluate(
        ...
        for extracted_file in &build.files {
                ...
                .chain(&extracted_file.resolved_reexports)
            {
                ...
                let target_tags = runtime.collect_target_tags(
                ...
                let target_candidate = Candidate {
                    ...
                    context: target
                        .node
                        .and_then(|node| match &build.module_graph.graph[node] {
...
impl CompiledFilter {
    ...
    fn matches(
        ...
        if let Some(matcher) = &self.tag_not
            && candidate
                .tags
                .is_some_and(|tags| tags.iter().any(|tag| matcher.is_match(tag)))
        {
            return false;
        }
        ...
        if let Some(matcher) = &self.profiles {
            ...
            if !profile_labels(context)
                .iter()
                .any(|label| matcher.is_match(label))
            {
                return false;
            }
        }
...
impl<'a> RuleRuntime<'a> {
    fn new(build: &'a GraphBuildResult, config: &OxgraphConfig, profile: EntrypointProfile) -> Self {
        Self {
            build,
            profile,
    ...
    fn is_reachable_from_query(&self, candidate: NodeIndex, query: &ReachQuery) -> bool {
        self.seed_nodes_for_query(query)
            .iter()
            .any(|seed| *seed == candidate || self.forward_reachable_from(*seed).contains(&candidate))
    }
    ...
    fn seed_nodes_for_query(&self, query: &ReachQuery) -> FxHashSet<NodeIndex> {
            ...
            match &self.build.module_graph.graph[index] {
                ModuleNode::File {
                    ...
                    if query.matcher.is_match(relative_path)
                        ...
                        || workspace
                            .as_deref()
                            .is_some_and(|value| query.matcher.is_match(value))
                        || package
                            .as_deref()
                            .is_some_and(|value| query.matcher.is_match(value))
                    {
        ...
        self.reach_seed_cache
            .borrow_mut()
            .insert(query.key.clone(), nodes.clone());
        nodes
    }
...
fn build_rule_contexts(build: &GraphBuildResult, config: &OxgraphConfig) -> RuleContexts {
...
fn edge_target(build: &GraphBuildResult, edge: &ResolvedEdge) -> EdgeTarget {
        ...
        && let Some(target_file) = build.find_file(target_path.to_string_lossy().as_ref())
    {
        return EdgeTarget {
            ...
            node: build
                .module_graph
                .file_node(&target_file.file.path.to_string_lossy()),
        };
    }
...
fn package_tags_from_ownership(
    ...
    for TeamConfig { packages, tags: team_tags, .. } in teams.values() {
        ...
        for package in packages {
            tags.entry(package.clone())
                .or_default()
                .extend(team_tags.iter().cloned());
        }
    }
...
impl OverrideTagMatcher {
    fn matches(&self, relative_path: &str, workspace: Option<&str>) -> bool {
        let file_ok = self
            .file_matcher
            .as_ref()
            .is_none_or(|matcher| matcher.is_match(relative_path));
    ...
    fn matches_workspace_only(&self, workspace: &str) -> bool {
        self.file_matcher.is_none()
            && self
                .workspace_matcher
                .as_ref()
                .is_some_and(|matcher| matcher.is_match(workspace))
    }
}

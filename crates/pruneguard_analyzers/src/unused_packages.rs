...
pub fn analyze(
    ...
    for seed in &build.entrypoint_seeds {
        ...
        if active && let Some(workspace) = &seed.workspace {
            candidate_workspaces.insert(workspace.clone());
        }
    }
    ...
    for workspace in build.discovery.workspaces.values() {
        if reachable_workspaces.contains(&workspace.name)
            || !candidate_workspaces.contains(&workspace.name)
        {
            continue;
        }

        let package_name =
            workspace.manifest.name.clone().unwrap_or_else(|| workspace.name.clone());
        ...
        findings.push(make_finding(
            ...
            format!("Package `{package_name}` has no reachable source files or entrypoints."),
            vec![Evidence {
                ...
                line: None,
                description:
                    "No active entrypoint or reachable package edge reaches this workspace package."
                        .to_string(),
            }],

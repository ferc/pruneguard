...
pub fn collect_changed_scope(
    ...
    let output = git_stdout(project_root, &args.iter().map(String::as_str).collect::<Vec<_>>())?;
    ...
    for line in output.lines().filter(|line| !line.trim().is_empty()) {
        ...
        if status.starts_with('R') {
            let from = parts.next().ok_or_else(|| {
                GitError::ParseError(format!("missing rename source in `{line}`"))
            })?;
            let to = parts.next().ok_or_else(|| {
                GitError::ParseError(format!("missing rename destination in `{line}`"))
            })?;
            scope.renamed.push(RenamedPath { from: PathBuf::from(from), to: PathBuf::from(to) });
            continue;
        }
...
fn git_stdout(project_root: &Path, args: &[&str]) -> miette::Result<String> {
    ...
    String::from_utf8(output.stdout).map_err(|err| GitError::CommandFailed(err.to_string()).into())
}
